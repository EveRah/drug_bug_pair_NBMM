...
#################### MIXED-EFFET MODEL with standardized data ##################
  ...
  # Get names of significant co-variables form univariate analysis
  
  setwd(paste0("Results_model/Results_model_", pair))
    significant_cov_df <- read.csv(paste0("significant_cov_uni_", pair,".csv"), header = T)
    significant_cov_list <- significant_cov_df[,2]
  setwd(path) #go back to directory
  
  # Functions to get regression formula with the covariables
  
  add_backticks = function(x) { paste0("`", x, "`") }
  cov_lm_formula = function(x) { paste(add_backticks(x), collapse = " + ") }
  build_lm_formula = function(x){ as.formula(paste0("Resistant ~ 1 + ", cov_lm_formula(x), " + (1 | Year_factor) + (1 | Country)")) }
  
  f_multi = build_lm_formula(significant_cov_list) #call final function

## MULTIVARIABLE with SIGNIFICANT co-variables model ----
...
## Backward selection
      ...
      y <- significant_cov_list
  
      for( i in 1:length(significant_cov_list)) {
        
        if (coef(summary(mixed_mod_log))[2:(1+length(y)),4] > 0.05) {
         
          if (length(y) > 1) {
            #get name of co-variable with highest p-value
            drop_cov <- names(which.max(coef(summary(mixed_mod_log))[2:(1+length(y)),4]))

            significant_cov_list_new <- significant_cov_list[-which(significant_cov_list %in% drop_cov)]

            f_multi = build_lm_formula(significant_cov_list_new)
            
            #new model with less covariables
            mixed_mod_log <- lme4::glmer.nb(
              f_multi
              ,
              data = df_na_omit,
...
##################### Store output of multivariable model #####################
...
# Dispersion factor theta

theta <- getME(mixed_mod_log, "glmer.nb.theta")
print(theta)

# Variance Inflation Factor VIF --> only if more than 1 covariable (so number of fixed effects is more than 2, including the intercept)

if(length(fixef(mixed_mod_log)) > 2) {
  vif <- vif(mixed_mod_log)
}

###################### Save outputs of model into file ########################

# create file to store all models' outputs by pair in the "Results_model" file

  dir.create(paste0("Results_model/Results_final_model_", pair))
  setwd(paste0("Results_model/Results_final_model_", pair))

# Save results of null model in a text file

sink(paste0("Results_multi_model_", pair,".txt"))
